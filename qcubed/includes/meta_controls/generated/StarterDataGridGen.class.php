<?php
	 ...
	 * listing a collection of Starter objects.  It includes
	 ...
	 *
	 */
	class StarterDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this Starter datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Starter object, or it can be any QQNode extending from QQN::Starter().
		 *
		 ...
		 * sorting using two fields from the Starter object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Starter
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Starter::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from Starter, given the clauses above
			$this->DataSource = Starter::QueryArray($objConditions, $objClauses);
		}

		/**
		 * Used internally by the Meta-based Add Column tools.
		 *
		 * Given a QQNode or a Text String, this will return a Starter-based QQNode.
		 * It will also verify that it is a proper Starter-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'starter') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "starter".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Starter()->Id;
				case 'Email': return QQN::Starter()->Email;
				default: throw new QCallerException('Simple Property not found in StarterDataGrid content: ' . $mixContent);
